...
class HoleAnalyzerGUI(QMainWindow):
    def __init__(self):
        ...
        self.tree.setHeaderLabels(["Feature", "Radii", "Length", "Position", "Direction"])
        ...
        try:
            ...
            for dim, tag in gmsh.model.getEntities(2):
                if "Cylind" in gmsh.model.getType(dim, tag):
                    ...
                    raw_cylinders.append({"tag": tag, "center":center,"c1": c1, "c2": c2, "radius": r1, "length": length, "dir": unit_v})
            ...
            for i, cy1 in enumerate(raw_cylinders):
                ...
                for j, cy2 in enumerate(raw_cylinders):
                    ...
                    d = np.linalg.norm(cy1["center"]-cy2["center"])
                    if d < 1e-3:
                        current.append(cy2); used.add(cy2["tag"])
                
                all_holes.append({
                    "radii": tuple(round(float(x["radius"]), 3) for x in current), 
                    "length": tuple(round(float(x["length"]), 3) for x in current), 
                    "pos": [float(x) for x in current[0]["center"]],
    ...
    def export_to_csv(self):
        ...
        try:
            with open(path, mode='w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["Type", "Instance", "Radii", "Length", "PosX", "PosY", "PosZ", "DirX", "DirY", "DirZ"])
                ...
                for i, (key, instances) in enumerate(groups.items()):
                        for j, inst in enumerate(instances):
                            # We iterate through the segments of each hole instance
                            for seg_idx, segment in enumerate(inst['segments']):
                                writer.writerow([
                                    f"Type {i+1}", 
                                    f"Instance {j+1}",
                                    round(float(segment["radius"]), 4), # Numeric Radius
                                    round(float(segment["length"]), 4), # Numeric Length
                                    inst['pos'][0], 
                                    inst['pos'][1], 
                                    inst['pos'][2],
                                    inst['dir'][0], 
